...
table_colors = cnx.session.sql("select color_or_style from catalog_for_website")
...
table_prod_data = cnx.session.sql("select file_name, price, size_list, upsell_product_desc, file_url from catalog_for_website where color_or_style = '" + option + "';")
